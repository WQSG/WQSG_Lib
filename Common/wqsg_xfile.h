...
{
	size_t rt = 0;
	if( (fileOffset < fileSize) && buffer )
	{
		size_t 可用len = fileSize - fileOffset;
		rt = (可用len >= readSize)?readSize:可用len;
		memcpy( buffer , (u8*)fileBuf + fileOffset , rt );
	}
	return rt;
}

inline size_t WWW_WQSG_WriteFile( void*const fileBuf , const size_t fileSize , const size_t fileOffset , void const*const buffer , const size_t writeSize )
{
	size_t rt = 0;
	if( (fileOffset < fileSize) && buffer )
	{
		size_t 可用len = fileSize - fileOffset;
		rt = (可用len >= writeSize)?writeSize:可用len;
		memcpy( (u8*)fileBuf + fileOffset , buffer , rt );
	}
	return rt;
}
class CCrc32
...
//------------------------------------------
class CWQSG_memFile : public CWQSG_IFile
{
	void*	m_mem;
	size_t	m_pointer;
	size_t	m_FileSize;
...
//------------------------------------------
#include "wqsg_xfile_inline.h"

#endif //__WQSG_xFile_H__
